# PH Private Holdout — External Blind Review Instructions

## 목적

이 packet은 PH01–PH30 법률 질문에 대해 검색된 판례가 질문의 핵심 쟁점을 얼마나 직접적이고 유용하게 다루는지 외부 reviewer가 독립적으로 평가하기 위한 것입니다. D/A6/AO arm 비교를 위한 평가이며, reviewer에게는 arm 정보가 제공되지 않습니다.

## 입력

`test/private/ph-holdout/blind_packet.json`의 각 sample에는 다음 정보만 있습니다.

- `sample_id`: 평가 결과를 연결하기 위한 식별자
- `question_id`: 질문 식별자
- `question_text`: 원 질문
- `provider_id`: 법제처 판례 식별자
- `source_locator`: 검증된 법제처 원문 링크
...
2. `source_locator`의 법제처 원문을 독립적으로 확인합니다.
...
```text
test/private/ph-holdout/blind_review_scores.jsonl
```

`sample_id`는 packet에 있는 값을 그대로 사용하고, 누락·중복·임의 sample 추가를 하지 않습니다. 외부 reviewer는 arm을 추측하거나 제품 결정을 대신 내리지 않습니다. 결과 JSONL을 반환하면 이후 schema 검증과 unmask는 별도 절차로 수행합니다.
